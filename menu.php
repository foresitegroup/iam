<ul class="clearfix">
  <li>
    <a href="<?php echo $TopDir; ?>free-trial.php">FREE TRIAL</a>
    <ul>
      <li><a href="<?php echo $TopDir; ?>download-iam.php">IAM Demo</a></li>
      <li><a href="<?php echo $TopDir; ?>download-iam-pro.php">IAM Pro Demo</a></li>
    </ul>
  </li>
  <li>
    <a href="<?php echo $TopDir; ?>why-iam.php">WHY IAM?</a>
    <ul>
      <li><a href="<?php echo $TopDir; ?>tour.php">Tour</a></li>
      <li><a href="<?php echo $TopDir; ?>features.php">Features</a></li>
      <li><a href="<?php echo $TopDir; ?>secure.php">Secure Software</a></li>
      <li><a href="<?php echo $TopDir; ?>unified-account-management.php" class="two-line">Unified Account<br>Management</a></li>
    </ul>
  </li>
  <li><a href="<?php echo $TopDir; ?>products">PRODUCTS</a></li>
  <li>
    <a href="<?php echo $TopDir; ?>learn.php">LEARN</a>
    <ul>
      <li><a href="#">Quick Start Guide</a></li>
      <li><a href="#">Tutorials</a></li>
      <li><a href="<?php echo $TopDir; ?>faq.php">FAQ</a></li>
    </ul>
  </li>
  <li>
    <a href="<?php echo $TopDir; ?>support.php">SUPPORT</a>
    <ul>
      <li><a href="<?php echo $TopDir; ?>online-support.php">Online Support</a></li>
      <li><a href="#">Manuals</a></li>
      <li><a href="#">Forum</a></li>
      <li><a href="<?php echo $TopDir; ?>products/blog/">Blog</a></li>
    </ul>
  </li>
  <li class="footer-only">
    <a href="<?php echo $TopDir; ?>about-us.php">ABOUT US</a>
    <ul>
      <li><a href="<?php echo $TopDir; ?>partners.php">Partners</a></li>
      <li><a href="<?php echo $TopDir; ?>testimonials-and-awards.php">Testimonials</a></li>
      <li><a href="<?php echo $TopDir; ?>privacy-policy.php">Privacy Policy</a></li>
      <li><a href="<?php echo $TopDir; ?>testimonials-and-awards.php">Awards &amp; Reviews</a></li>
    </ul>
  </li>
  <li class="search">
    <a href="<?php echo $TopDir; ?>search.php"><i class="fa fa-search"></i><span> SEARCH</span></a>
  </li>
  <li class="cart">
    <a href="<?php echo $TopDir; ?>products/cart"><i class="fa fa-shopping-cart"></i> CART</a>
  </li>
</ul>